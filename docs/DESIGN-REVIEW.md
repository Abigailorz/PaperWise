...
### 2.1 审核结果回流：revise-until-pass 循环

**现状**：`agents/orchestrator.py` 的 Pipeline 是 Analyst → Writer → Reviewer
串行执行。Reviewer 把发现写入 `review/findings.md` 后流程就结束了，
报告不会根据审核意见修改；HallucinationDetector 的"一票否决"也没有接入主流程。

**方案**：
1. Reviewer 输出结构化 findings（严重度 + 证据 + 修改建议）
2. 若有 critical/major 问题 → 把 findings 作为 feedback 注入 Writer 重写对应章节
3. 最多 3 轮，仍不通过则标记"需人工审核"并随报告输出
4. 最终报告的 `report.md` 附带审核通过记录

**涉及文件**：`agents/orchestrator.py`、`evaluation/__init__.py`、`generators/report.py`
...
**方案**：
1. run_agent_tests 接入 PassKEvaluator（k=3~5 起步，成本可控）
2. 结果写入 `workspace/test_runs/` 并生成可比较的 JSON
3. 扩展 `tests/test_data/papers/`：加入 2-3 篇不同领域（CV/NLP/系统）的小型论文
4. 后续接入 CI（GitHub Actions 或本地脚本），每次改动跑一遍能力基线
...
**方案**：启动时扫描 `workspace/session_*/` 下的 `.sessions/paperwise.db`，
按需惰性恢复；`GET /api/sessions` 返回历史会话列表。
...
**方案**：为 `UserMemory` 增加 `consolidate()`：按置信度 + 时间戳降级旧记忆、
把低价值 experience 折叠进 profile、输出压缩报告；由 `set_timer` 或
会话启动时低频触发。
...
**方案**：实现一个进程内 `AgentBus`（asyncio.Queue + 注册表），
支持 Manager → Worker 任务下发、Worker → Manager 状态回报；
...
**方案**：新增 `discover_tool(query)` 工具——按关键词返回匹配工具的
完整定义（参数示例 + 边界说明），让 Agent 在任务中自行发现新能力。

### 3.4 RAPTOR / GraphRAG 索引持久化

**现状**：每次会话 `build_raptor_tree()` / `build_knowledge_graph()` 全量重建，
LLM 调用成本高，且只覆盖前 10 篇文档的前 2000 字符。

**方案**：索引结果序列化到知识库存储（sqlite），
按 doc_id 做增量更新；对已有摘要节点标记来源，避免重复生成。
...
**方案**：`exec_tools.py` 增加命令预处理层——维护常见别名表
...
**方案**：文件写入前检查目标是否被其他 Agent 声明占用（写入 `locks.json`），
...
| 阶段 | 内容 | 预期效果 |
|------|------|---------|
| A（可靠性） | revise-until-pass、评估落地、Web 会话恢复、记忆整合 | 输出可信、可量化、可恢复 |
| B（能力） | Agent 消息总线、调度器、工具自发现、索引持久化 | 多 Agent 真协作、主动服务可用 |
| C（产品） | UI 增强、arXiv、用户系统、评估 Dashboard | 可对外使用 |
