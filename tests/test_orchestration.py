...
Covers:
- TaskClassifier route decisions (rule-based, no LLM).
- TaskRoute schema and two-stage fallback flags.
...
from paperwise.orchestration.types import TaskComplexity, TaskType
...
def test_smart_orchestrator_escalates_on_failure(tmp_workspace):
    """Ambiguous simple tasks are routed with escalate_on_failure so a failed
    single-agent attempt can fall back to the DAG.

    This smoke test checks that the routing flag is set and the orchestrator
    ...
    async def _inner():
        llm = MockLLMClient([
            ...
            LLMResponse(content="It uses a graph neural network."),
        ])
        ...
        paper_dir.mkdir(exist_ok=True)
        (paper_dir / "text.md").write_text(
            "# TestPaper\nWe use a graph neural network.", encoding="utf-8")
        ...
        result = await orchestrator.run("Explain the method.", paper_dir=paper_dir)
